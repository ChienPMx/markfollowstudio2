...
// SetGlobalThemeManager sets the global theme manager
...
// FadeAnimation Fade-in and fade-out animation
...
// GlassmorphismCard Glassmorphism effect card
func GlassmorphismCard(title, subtitle string, content fyne.CanvasObject, isDark bool) *fyne.Container {
	...
	if isDark {
		...
		bgColor = color.NRGBA{R: 30, G: 41, B: 59, A: 180} // Translucent dark background
		...
		// Light theme glassmorphism effect
		bgColor = color.NRGBA{R: 255, G: 255, B: 255, A: 180} // Translucent white background
...
// TransparentCard Transparent effect card
...
// StyledCard Styled card - optimized version
...
func StyledSelect(options []string, selected func(string)) *widget.Select {
	...
	for _, option := range options {
		if len(option) > 8 {
			...
			// Add extra spaces to extend width
			padding := "                          "
			if len(maxOption) < 20 {
...
// StyledPasswordEntry Styled password input box
...
// DividedContainer Divided container
...
// createDivider Creates divider line
...
// ProgressWithLabel Progress bar with label
...
// UpdateProgressLabel Updates progress bar label
...
// SwitchContent Switches content
...
// Custom tappable object to avoid default button style
type tappableObject struct {
	widget.BaseWidget
	rect    *canvas.Rectangle
	onTap   func()
	onHover func(bool) // Hover callback function
}
