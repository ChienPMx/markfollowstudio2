...
* يجب وضع هذا الملف داخل مجلد `config`. يجب أن تكون البنية الصحيحة للدليل العامل كما يلي:
  ...
  /── config/
  │   └── config.toml
  └── MarkFlow Studio.exe (ملفك التنفيذي)
  ```
...
1. قم بتنزيل الخطوط المطلوبة، مثل [Microsoft YaHei](https://modelscope.cn/models/Maranello/MarkFlow Studio_dependency_cn/resolve/master/%E5%AD%97%E4%BD%93/msyh.ttc) و [Microsoft YaHei Bold](https://modelscope.cn/models/Maranello/MarkFlow Studio_dependency_cn/resolve/master/%E5%AD%97%E4%BD%93/msyhbd.ttc).
...
### 4. على macOS، التطبيق لا يبدأ ويظهر خطأ مثل "MarkFlow Studio تالف ولا يمكن فتحه."
...
2. اكتب الأمر `xattr -cr` متبوعًا بمسافة، ثم اسحب ملف `MarkFlow Studio.app` من نافذة Finder إلى Terminal. سيبدو الأمر كما يلي:
    ```
...
نعم، يمكنك تكوين MarkFlow Studio لاستخدام أي LLM محلي يوفر نقطة نهاية API متوافقة مع OpenAI.
...
لا. حاليًا، يقوم MarkFlow Studio بإنشاء **ترجمات مشفرة**، مما يعني أنها تُحرق مباشرة في إطارات الفيديو. التطبيق **لا يقدم خيارات لتخصيص نمط الترجمة**؛ يستخدم نمطًا محددًا مسبقًا.

للحصول على تخصيص متقدم، الحل الموصى به هو:

1. استخدام MarkFlow Studio لإنشاء ملف الترجمة `.srt` المترجم.
...
### 11. لدي بالفعل ملف `.srt` مترجم. هل يمكن لـ MarkFlow Studio استخدامه فقط لأداء الدبلجة؟
